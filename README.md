[![Review Assignment Due Date](https://classroom.github.com/assets/deadline-readme-button-22041afd0340ce965d47ae6ef1cefeee28c7c493a6346c4f15d667ab976d596c.svg)](https://classroom.github.com/a/dbvp16aO)
[![Open in Codespaces](https://classroom.github.com/assets/launch-codespace-2972f46106e565e64193e422d61a12cf1da4916b45550586e14ef0a7c637dd04.svg)](https://classroom.github.com/open-in-codespaces?assignment_repo_id=20417696)
# Project Highlights:
#### -Implemented Linux kernel code in C in a team of 3 to track, monitor, and make reservations for simulated tasks using rate monotonic and partitioned EDF scheduling
#### -Evaluated performance by calculating end-to-end latency of simulated task chains and tracking budget overspend

## Note: This project was completed as the main focus of my CS596 Real Time Systems class and included work from the following people:
#### -Jake de los Reyes (SDSU CS student; me)
#### -Justin Pelak (SDSU CS student; group member)
#### -Cameron Ho (SDSU CS student; group member)
#### -Dr. Hyunjong Choi (SDSU Assistant Professor of Computer Science; instructor)
